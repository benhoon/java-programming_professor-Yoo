...
public class Zip_quarter {
	...
	Zip_quarter(String U_InputFile_Path1, String U_InputFile_Path2, String U_InputFile_Path3, String U_InputFile_Path4, String U_Outputfile_Path){
		U_InputFile = new File(U_InputFile_Path1);
		...
		for (column = 0; column <= height-1; column+=2) {
		...
		U_InputFile = new File(U_InputFile_Path2);
		try {
			SourceImage = ImageIO.read(U_InputFile);
		}
		catch(IOException e) {
		...
		for (column = 0; column <= height-1; column+=2) {
			for (row = 0; row <= width-1; row+=2) {
				color = new Color(SourceImage.getRGB(row, column));
				TargetImage.setRGB(width/2+row/2, column/2, color.getRGB());
			}
		}
		
		U_InputFile = new File(U_InputFile_Path3);
		try {
			SourceImage = ImageIO.read(U_InputFile);
		}
		catch(IOException e) {
		...
		for (column = 0; column <= height-1; column+=2) {
			for (row = 0; row <= width-1; row+=2) {
				color = new Color(SourceImage.getRGB(row, column));
				TargetImage.setRGB(row/2, height/2 + column/2, color.getRGB());
			}
		}
		
		
		
		U_InputFile = new File(U_InputFile_Path4);
		try {
			SourceImage = ImageIO.read(U_InputFile);
		}
		catch(IOException e) {
		...
		for (column = 0; column <= height-1; column+=2) {
			for (row = 0; row <= width-1; row+=2) {
				color = new Color(SourceImage.getRGB(row, column));
				TargetImage.setRGB(width/2 + row/2, height/2 + column/2, color.getRGB());
			}
		}
